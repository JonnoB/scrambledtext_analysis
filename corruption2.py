...
def __():
    ...
    import time 
    import evaluate
    from lm_support_functions import training_prompt, compute_metric, infer_on_test_set, cleaning_prompt_formatter
...
def __(
    ...
    if not os.path.exists(_file_path):
        ...
        corruption_probs = ProbabilityDistributions().load_from_json('data/learned_corruption_distribs.json')
        ...
        for wer in np.arange(0.1, 1.1, 0.1):
            for cer in [0.05]: #np.arange(0.1, 1, 0.1)
                ...
                corruption_function = CorruptionEngine(corruption_probs.conditional, 
                ...
                synth_data['ocr_text'], synth_data['observed_wer'], synth_data['observed_cer'], synth_data['observed_effective_cer'] = zip(
                ...
                synth_data['full_prompt'] = synth_data.apply(lambda row: training_prompt(row, 'ocr_text', 'gt_text', tokenizer)['full_prompt'], axis=1)
                synth_data['total_tokens'] = synth_data['full_prompt'].apply(lambda text: len(tokenizer.encode(text)))
                ...
                results.append({
                    ...
                    'observed_effective_cer': synth_data['observed_effective_cer'].mean()
                })
...
def __(os, plt, results_df, save_appendix, sns):
    ...
    _heatmap_data = results_df.pivot(index="target_wer", columns="target_cer", values="max_tokens")

    # Plot the heatmap
    plt.figure(figsize=(10, 8))
    sns.heatmap(_heatmap_data, annot=True, fmt=".1f", cmap="YlGnBu", linewidths=.5)
...
def __(os, plt, results_df, save_appendix, sns):
    # Pivot the DataFrame to create a grid for the heatmap
    _heatmap_data = results_df.pivot(index="target_wer", columns="target_cer", values="observed_effective_cer")
