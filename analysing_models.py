...
def __():
    ...
    synth_cor_stats_df['target_wer'] = (synth_cor_stats_df['target_wer']*100).astype(int)
    synth_cor_stats_df['target_cer'] = (synth_cor_stats_df['target_cer']*100).astype(int)

    synth_cor_stats_df['target_cer'] = np.where(synth_cor_stats_df['target_cer']==0, 5, synth_cor_stats_df['target_cer'])
    return (
        dotenv,
        np,
        os,
...
def __(os, pd):
    def process_experiment_results(folder_path, min_cer=None, max_cer=None):
        ...
        _cer_vals_df[['target_cer', 'target_wer']] = _cer_vals_df['model'].str.extract(r'cer_(\d*)_wer_(\d*)')
...
def __(model_performance_df, plt, process_experiment_results, sns):
    ...
    melted_df = cer_vals_df.melt(id_vars=['target_cer'], value_vars=['wer', 'cer'], 
...
def __(
    ...
    cer_wer_vals_df = process_experiment_results('data/cer_wer_exp', min_cer=_min_cer_val)
    ...
    llama_base = process_single_experiment_result(
        os.path.join('data/results','ncse_test_recovered_base_llama.csv'), min_cer = _min_cer_val)




    print(f"Total obs {cer_wer_vals_df['total_obs'].min()}. Orginal CER: {cer_wer_vals_df['cer_orig'].min().round(2)}. Orginal WER: {cer_wer_vals_df['wer_orig'].min().round(2)}")

    _type = 'cer'
    _orig_llama_met = model_performance_df.loc[model_performance_df['model']=='base llama',_type].to_list()[0]
    _orig_base_met = model_performance_df.loc[model_performance_df['model']=='original',_type].to_list()[0]
...
def __(os, pd, plt, sns):
    llama_base_full = pd.read_csv(os.path.join('data/results','ncse_test_recovered_base_llama.csv'))
...
def __(
    ...
    _temp = pd.concat([cer_wer_vals_df, cer_vals_df.loc[cer_vals_df['target_cer']<=40]], ignore_index=True)
...
def __(
    ...
    _cer_wer_vals_df1 = process_experiment_results(_folder_path, min_cer=balance_value, max_cer=None)
    _cer_wer_vals_df1['type'] = 'paired'


    _cer_vals_df1 = process_experiment_results('data/cer_exp/results', min_cer=balance_value)
    _cer_vals_df1['type'] = 'uniform'
    _cer_vals_df1 = _cer_vals_df1.loc[_cer_vals_df1['target_cer']<=40] #get rid of really high values as they are junk

    sns.scatterplot(data=pd.concat([_cer_wer_vals_df1, _cer_vals_df1], ignore_index=True), x='cer', y='wer', hue=_hue, ax=_ax1, s = 100, palette='viridis', style = 'type')
    ...
    _llama_base1 = process_single_experiment_result(
        os.path.join('data/results', 'ncse_test_recovered_base_llama.csv'), min_cer=balance_value, max_cer=None)
    ...
    _cer_wer_vals_df2 = process_experiment_results(_folder_path, min_cer=None, max_cer=balance_value)
    _cer_wer_vals_df2['type'] = 'paired'

    _cer_vals_df2 = process_experiment_results('data/cer_exp/results', max_cer=balance_value)
...
def __(plt, process_experiment_results, sns, synth_cor_stats_df):
    ...
    _cer_wer_vals_df1['type'] = 'paired'
    _cer_wer_vals_df1 = _cer_wer_vals_df1 = _cer_wer_vals_df1.loc[_cer_wer_vals_df1['cer']<0.0189]


    _cer_wer_vals_df1 = _cer_wer_vals_df1.merge(synth_cor_stats_df[['target_wer', 'target_cer', 'observed_effective_cer']], 
                       on = ['target_wer', 'target_cer'], how = 'left')

    _cer_wer_vals_df1.rename(columns = {'observed_effective_cer':'o_cer'}, inplace = 'True')

    sns.scatterplot(data=_cer_wer_vals_df1, x='cer', y='wer', hue=_hue, s = 100, palette='viridis')
...
def __(os, pd):
    ...
    cer_wer_results_df[['target_cer', 'target_wer']] = cer_wer_results_df['data_type'].str.extract(r'cer_(\d{2})_wer_(\d{2})')
...
def __(cer_wer_results_df, plt, sns):
    ...
    _plot_df = cer_wer_results_df.loc[cer_wer_results_df['file_name']==_file_name].reset_index()

    _variable = 'cer'
    # Create pivot table
    _pivot = _plot_df.pivot_table(values=_variable, index='target_wer', columns='target_cer')


    _sns_heatmap = sns.heatmap(_pivot,  cmap='YlOrRd', cbar_kws={'label': _variable})
    plt.title(f"Original; CER {_plot_df.loc[0, 'cer_orig'].round(2)}, WER {_plot_df.loc[0, 'wer_orig'].round(2)}")
...
def __(
    ...
    _pivot = _temp.pivot_table(values='cer', index='tokens', columns='token_length', )
    ...
    sns.lineplot(data=_temp, x='tokens_power', y='cer', hue="token_length", ax=axes[0], legend=False, palette='viridis')
    axes[0].set_xlabel("Number of tokens in training set where Tokens = $100\cdot2^x$")

    # Plot the boxplot on the left
    sns.boxplot(data=_temp, x='token_length', y='cer', hue='token_length', ax=axes[1], palette='viridis')
    axes[1].set_xlabel("Number of tokens per observation")
    legend = axes[1].get_legend()
    legend.set_title("Tokens per obs")

    # Add a joint title for both plots
    plt.suptitle("CER by tokens per observation and total tokens in dataset", fontsize=16)
...
def __(cer_wer_results_df):
    _file_name = 'artid_494321_periodical_ewj_issue_ewj_01051860_page_number_49.txt'
    _temp = cer_wer_results_df.loc[cer_wer_results_df['file_name']==_file_name].reset_index()
