...
def __(os, pd, wer_cer_loss):
    ...
    _folder_path = 'data/results'
    for file in os.listdir(_folder_path):

        temp = pd.read_csv(os.path.join(_folder_path, file))
        temp = temp[['wer', 'cer']].median().to_frame().T
        temp['model'] = file.replace(".csv", "")
    ...
    return file, model_performance_df, temp
...
def __(model_performance_df, plt, sns):
    ...
    base_values = model_performance_df[model_performance_df['model'] == 'ncse_test_recovered_base_llama']
