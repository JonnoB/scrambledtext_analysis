...
if corruption_type == 'cer':
    ...
    corruption_probs.modify_and_renormalize_probs(column='correct', desired_value=corruption_args['cer'], inplace=True)
    ...
    synth_data['ocr_text'], synth_data['cer'] = zip( *synth_data['gt_text'].apply(lambda text:corruption_function.corrupt_text(text)))

    output_file_name = f"""{file_name}_{int(corruption_args['cer']*100)}.csv"""

elif corruption_type =='cer_wer':

    corruption_function = WERBasedCorruptionEngine(corruption_probs.conditional, corruption_probs.substitutions, corruption_probs.insertions)

    synth_data['ocr_text'], synth_data['wer'], synth_data['cer'] = zip(
    *synth_data['gt_text'].apply(lambda text:corruption_function.corrupt_text_with_wer_cer(text, target_wer = corruption_args['wer'], target_cer = corruption_args['cer']))
)
